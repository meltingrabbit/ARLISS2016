...
int JudgeLandingLightModeCheckGpsHeight();
...
//着地判定
// long LIGHT_JUDGE_START_DELAY_SEC = 120;		// ロケット封入から，光判定までの空き時間
// long LIGHT_JUDGE_DELAY_SEC = 2400;		//放出判定した場合、放出判定してから着地と判断するまでの時間
// long LIGHT_JUDGE_TIMEOUT_SEC = 7200;
// long GPS_LIGHT_JUDGE_TIMEOUT_SEC = 3600;		//単位は(秒) この二つのTIMEOUT_SECの和が、着地判定自体のタイムアウト時間
...
double DEFAULT_ADJUST_MOTER_POWER = 10.0 / 13.0;
...
const double MAGNET_DECLINATION_DEG = -14.0;					// ネバダ州
int Magnet_X12 = 0;
int Magnet_Z12 = 0;
int Magnet_Y12 = 0;
double MAGNET_X_OFFSET = 215.0;
double MAGNET_Y_OFFSET = 98.0;
double MAGNET_XY_RATIO = 1.08;
...
// 最寄りの砂漠
// const long TARGET_LATITUDE = 40871044;
// const long TARGET_LONGITUDE = -119106671;

// ARLISS ゴール
const long TARGET_LATITUDE = 40885240;
const long TARGET_LONGITUDE = -119116105;

// Passing 南北境界
long PASSING_NS_BORDER = 40872000;
long PASSING_EW_BORDER = -119106265;
...
void setup() {
	...
	double magnetRad = GetMagnetRadNTimes(5);

	GoStraight(magnetRad, 1000000);
...
void loop() {
	...
	// 北側
	long passingLatitudes0[] = {TARGET_LATITUDE, TARGET_LATITUDE};
	long passingLongitudes0[] = {TARGET_LONGITUDE, TARGET_LONGITUDE};
	// 西側
	long passingLatitudes1[] = {40868500, 40868500};
	long passingLongitudes1[] = {TARGET_LONGITUDE, TARGET_LONGITUDE};
	// 東側
	long passingLatitudes2[] = {40868500, 40874136};
	long passingLongitudes2[] = {-119096324, -119099357};

	// for debug
	if (!(sizeof(passingLatitudes) == sizeof(passingLongitudes) && sizeof(passingLatitudes) == sizeof(passingLongitudes1) && sizeof(passingLatitudes) == sizeof(passingLongitudes0) && sizeof(passingLatitudes) == sizeof(passingLongitudes2) && sizeof(passingLatitudes) == sizeof(passingLatitudes0) && sizeof(passingLatitudes) == sizeof(passingLatitudes1) && sizeof(passingLatitudes) == sizeof(passingLatitudes2)  && sizeof(passingLatitudes) == 4 * numOfPassingPoint)) {
	...
	ClearGetGpsData();
	// とりあえず東西で
	if (Gps_Lat > PASSING_NS_BORDER) {			// 北
		// 北側落下
		MyPrintln(F("KITAKITAKITA"));
		LogSta("N");
		for (int i = 0; i < numOfPassingPoint; i++) {
			passingLatitudes[i] = passingLatitudes0[i];
			passingLongitudes[i] = passingLongitudes0[i];
		}
	} else if (Gps_Long < PASSING_EW_BORDER) {			// 東西
		// 西側落下
	...
	Passing_Latitude = passingLatitudes[0];
	Passing_Longitude = passingLongitudes[0];
	...
	MyPrintln(F("RunUpNearGoal(10)"));
	RunUpNearGoal(10);


	// カメラ初期化
	int error = InitializeCamera();
	if (error == 0) {
		LogSta("ICf");
...
void InitializeAll() {
	...
	while ( (SD.exists( FLIE_NAME )) ) {
		MyPrint(missionDir);
		MyPrintln(F(" exist..."));
		missionNo++;
		for (int i = 0; i<6; i++) {
			missionDir[i+2] = (missionNo/LongPow(10,5-i)) % 10 + '0';
			FLIE_NAME[i+2] = (missionNo/LongPow(10,5-i)) % 10 + '0';
		}
		MyDelay(50);
	}
	// SD.mkdir( missionDir );
...
void CheckAndCalibrateSensor() {
	...
	if (SENSOR_CEHCK) {
		...
		while(1) {
			...
			MyPrint(F("\n"));
			MyPrint(F("magnet avg deg : "));
			MyPrint(Rad2Deg(GetMagnetRadNTimes(5)));
			MyPrint(F("\n"));
...
void JudgeLanding() {
	...
	if (JudgeLandingLightMode() == 1) {
		...
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
			...
			if (JudgeLandingLightModeCheckGpsHeight() == 1) {
				break;
			}
...
int JudgeLandingLightModeCheckGpsHeight() {
	unsigned long time = millis();
	const int rangeGpsJudge = 2*1;		// 偶数にする 2*片側範囲
	const int averageNum = 3;			// 基準平均のサンプリング回数
	const int averageNumMin = 3;		// 基準平均のサンプリング最低回数
	...
	const double borderHeight = 1250;		// 判定着地標高
	int countGpsData = 0;
	...
	MyPrintln(F("After JudgeLandingLightMode Check Gps_Height"));
	LogSta("AJLLMCG");
	...
	if (countGpsData >= averageNumMin) {		//!!!判定準備!!!   (平均の時に一定数以上取れてたら)
		...
		LogComStrDbl("aG", averageGps);
		if (averageGps > borderHeight) {
			MyDelay(50);
			return 0;
		}
		if (DEBUG) MyPrintln(F("Gps Sampling Start, in CheckGpsHeight"));
		LogSta("GSSiG");
		countGpsData = 0;
		countNearGpsData = 0;
		for (int i = 0; i < sampleNum; i++) {
			if (ClearGetGpsData() == 1) {
				if (Gps_Height <= averageGps + rangeGpsJudge/2 && Gps_Height >= averageGps - rangeGpsJudge/2) {			//rangeの中にあるときに追加
					countNearGpsData++;
					if (DEBUG) MyPrintln(F("GpsData In range"));
				} else {
					if (DEBUG) MyPrintln(F("GpsData Out of range"));
				}
				countGpsData++;
			}
			LogGps();
		...
		if(countGpsData >= sampleNumMin){		//!!!判定!!!
			...
			if(rateNearGpsData >= rateJudge){
				...
				MyPrintln(F("CheckGpsHeight FAILED (low rate)"));
			}
		} else {
			MyPrintln(F("CheckGpsHeight FAILED (get GPS failed)"));
		}
	} else {
		MyPrintln(F("CheckGpsHeight FAILED (get averageGPS failed)"));
	}
	MyDelay(100);
	return 0;
}
...
void EscapeLandingPoint() {
	...
	if (DEBUG) MyPrintln(F("OP : BurstGoAhead 3s"));
	BurstGoAhead(3000);
	MyDelay(20);
	...
	if (DEBUG) MyPrintln(F("OP : GoStraight (Burst) 1, 20m"));
	Adjust_Moter_Power = 1;
	magnetRad = NormalizeRadSym(GetMagnetRadNTimes(5));
	GoStraight(magnetRad, Meter2Msec(20));
	...
	if (DEBUG) MyPrint(F("OP : Gps_Passing_Distance : "));
	if (DEBUG) MyPrintln(Gps_Passing_Distance);
	if (DEBUG) MyPrint(F("OP : Gps_Passing_Radian to Deg : "));
	if (DEBUG) MyPrintln( Rad2Deg(Gps_Passing_Radian) );

	errorDirection = NormalizeRadSym(Gps_Passing_Radian - magnetRad);
	if (errorDirection > 0) {
		magnetRad += (M_PI/2);
	} else {
		magnetRad += -(M_PI/2);
	...
	if (DEBUG) MyPrintln(F("OP : GO P 2, 20m"));
	GoStraight(magnetRad, Meter2Msec(20));
	MyDelay(1000);
...
void RunUpNearGoal(double residualError) {
	...
	while(1) {
		...
		goStraightMeter = min(goStraightMeter, 40);
...
void ApproachGoal(double residualError) {
	...
	while(1) {
		...
		// 進む
		goStraightMeter = Gps_Distance * 0.6;
		goStraightMeter = max(goStraightMeter, 1);
		goStraightMeter = min(goStraightMeter, 20);
		goStraightMeter = goStraightMeter - 0.5;			// SetMoter(170,0,170,0);分だけ引く
		goStraightMeter = max(goStraightMeter, 0);
...
void GoStraight(double targetRad, unsigned long msec) {
	...
	targetRad = NormalizeRadSym(targetRad - Deg2Rad(5));
	...
	while (millis() < startTime + msec) {
		...
		if ( (count % 10) == 0 ) {
			...
			if (DEBUG) Serial3.print(gainP*errorDirection, 6);
			if (DEBUG) MyPrint(F("\tI : "));
			if (DEBUG) Serial3.print(gainI*sumError, 6);
			if (DEBUG) MyPrint(F("\tsumError : "));
			if (DEBUG) Serial3.println(sumError, 6);
		}
...
int ExecuteMagnetCalibration(int magnetCalError) {
	...
	double PreMagnetXOffset = MAGNET_X_OFFSET;
	double PreMagnetYOffset = MAGNET_Y_OFFSET;
	double PreMagnetXyRatio = MAGNET_XY_RATIO;
	...
	while (1) {
		...
		if (count >= maxCount) {
			MyPrint(F("ExecuteMagnetCalibration failed"));
			LogSta("EMCf");
			MAGNET_X_OFFSET = PreMagnetXOffset;
			MAGNET_Y_OFFSET = PreMagnetYOffset;
			MAGNET_XY_RATIO = PreMagnetXyRatio;
			return -1;
		}
		magnetTry++;
		count++;
