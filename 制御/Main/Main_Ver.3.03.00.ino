...
double Adjust_Moter_Power = 0;		// モーターのパワーを動的に制御するため 主にでApproachGoalでBusrtするため
...
void setup() {
	...
	// コマンド打ち込み
/*	MyPrintln(F("Waiting for Start Cmd..."));
	time = millis();
	int cmd = 0;
...
void loop() {
	unsigned long time = 0;
	int count = 0;
	long passingLatitudes1[] = {40142716, 40142716, 40142716};
	long passingLongitudes1[] = {139987291, 139987291, 139987291};
	long passingLatitudes2[] = {40142716, 40142716, 40142716};
	long passingLongitudes2[] = {139987291, 139987291, 139987291};
OpenCasing();
	if (Mission_Status == 0) {
...
//			MyPrintln(F("waiting for LIGHT_JUDGE_START_DELAY_SEC"));
//			GetLight();				// ログ用
//			CapturePic();
...
void JudgeLanding() {
	...
	if (JudgeLandingLightMode() == 1) {
		...
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
			MyPrintln(F("waiting for LIGHT_JUDGE_DELAY_SEC"));
			GetLight();				// ログ用
			CapturePic();
			// GetGPSはCapturePicに内包
			CacheOff();
...
void JudgeLandingGpsLightMode() {
	...
	while(1) {			//!!!gps判定ループ!!!
		...
		countGpsData = 0;
		sumGpsHeight = 0;
		if (DEBUG) MyPrintln(F("Start GetGpsAverage"));
		...
		if (countGpsData >= averageNumMin) {		//!!!判定準備!!!   (平均の時に一定数以上取れてたら)
			if (DEBUG) MyPrintln(F("GetGpsAverageSuccess, sampling start"));
			...
			MyPrintln(F("GetGpsAverageFailed..."));
