...
long Gps_Lat;									//
long Gps_Long;									//dd.mmmmmmにすること
double Gps_Height;								//
double Gps_Distance;							//センサと目的地間距離
double Gps_Radian;								//センサから目的地までの方位
long Gps_Time;
const long TARGET_LATITUDE = 35713791;
const long TARGET_LONGITUDE = 139760198;		// dd ddd dddにすること！安田講堂塔真ん中(D, D)=(1602.1, 4.48286)
...
int GetGpsData() {
	...
	if (gpsArrLength == 20 && !strcmp(Gps_Raw_Arr[0], "$GPGGA")) {		// 送られてきたデータが20個で，$GPGGAならtrue
		if (strcmp(Gps_Raw_Arr[1], "NULL") && strcmp(Gps_Raw_Arr[3], "NULL") && strcmp(Gps_Raw_Arr[4], "NULL") && strcmp(Gps_Raw_Arr[5], "NULL") && strcmp(Gps_Raw_Arr[6], "NULL") && strcmp(Gps_Raw_Arr[7], "NULL") && strcmp(Gps_Raw_Arr[8], "NULL") && strcmp(Gps_Raw_Arr[16], "NULL")&& strcmp(Gps_Raw_Arr[17], "NULL")) {

			Gps_Lat = GpsLL2Deg(Gps_Raw_Arr[3], Gps_Raw_Arr[4]);			//char ddmm mmmm → long dddddddd
			Gps_Long = GpsLL2Deg(Gps_Raw_Arr[6], Gps_Raw_Arr[7]);
			Gps_Height = atol(Gps_Raw_Arr[16]) + 0.01 * CutStr2Long(Gps_Raw_Arr[17], 2);			//コピーできないので元配列から直接
			Gps_Time = atol(Gps_Raw_Arr[1]);

			if (strcmp(Gps_Raw_Arr[5], "N")) {			// S だとtrue
				Gps_Lat = -1 * Gps_Lat;					//南緯の場合負の値にする  一応南緯も対応...?(笑)
			}
			if (strcmp(Gps_Raw_Arr[8], "E")) {			// W だとtrue
...
long GpsLL2Deg(char *ddmmStr, char *mmmmStr){
	...
	long ans = gpsDegree*1000000 + (gpsMinute/60)*10000 + gpsMinuteDec/60;
	return ans;
}
...
//	double gapY = EARTH_RADIUS * gapLatRad;
	double gapLat = (TARGET_LATITUDE - Gps_Lat) * 1.0 / 10e6;		//差は最大でも100000と見積もって、doubleにする
	double gapLong = (TARGET_LONGITUDE - Gps_Long) * 1.0 / 10e6;

	double latRad = Deg2Rad(Gps_Lat);
