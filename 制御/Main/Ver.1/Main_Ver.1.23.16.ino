...
void GoStraight(double targetRad, unsigned long msec) {			//msecはミリ秒
	...
	int loopTime = 100;
	double magnetRad = 0;
	const int modePid = 1;		//1のときpid制御、0の時p制御
...
//		Serial.println(Rad2Deg( errorDirection ),6);
		...
		if(errorDirection > 0 && errorDirection < M_PI/2) {
//			if(DEBUG){
//				Serial.println("Turing RIGHT");
//			}
			SetMoter(255,0, 255 * (M_PI - inputPid) / M_PI,0);

		} else if(errorDirection < 0 && errorDirection > - M_PI/2) {
//			if(DEBUG){
...
int InitializeMagnet() {
	...
	int magnetCalError = 6;			// 地磁気キャリブレーションの許容エラー
