...
const double TARGET_LONGITUDE = 　139.457297;		//ddddddddにすること！安田講堂塔真ん中(D, D)=(1602.1, 4.48286)
...
int GetGpsData() {
	...
	if (gpsArrLength == 14 && !strcmp(Gps_Raw_Arr[0], "$GPGGA")) {		// 送られてきたデータが15個で，$GPGGAならtrue
		if (strcmp(Gps_Raw_Arr[3], "NULL") && strcmp(Gps_Raw_Arr[4], "NULL") && strcmp(Gps_Raw_Arr[6], "NULL") && strcmp(Gps_Raw_Arr[7], "NULL") && strcmp(Gps_Raw_Arr[16], "NULL")&& strcmp(Gps_Raw_Arr[17], "NULL")) {

			Gps_Lat = GpsLL2Deg(Gps_Raw_Arr[3],Gps_Raw_Arr[4]);			//char ddmm mmmm → long dddddddd
			Gps_Long = GpsLL2Deg(Gps_Raw_Arr[6],Gps_Raw_Arr[7]);
			Gps_Height = Gps_Raw_Arr[16] + 0.01 * CutStr2Deg(Gps_Raw_Arr[6],2);			//コピーできないので元配列から直接
...
long GpsLL2Deg(char *ddmmStr, char *mmmmStr){
	...
	long ans = gpsDegree*1000000 + gpsMinute*10000 + gpsMinuteDec;
	return ans;
}
...
//	double gapY = EARTH_RADIUS * gapLatRad;
	long gapLat = TARGET_LATITUDE - Gps_Lat;		//値は最大でも1000000と見積もる
	long gapLong = TARGET_LONGITUDE - Gps_Long;
	...
	Gps_Radian = NormalizeRad( atan2(gapX, gapY) );			//代入値は分母分子1000000倍されている
}
