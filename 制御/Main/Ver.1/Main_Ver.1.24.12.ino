...
const long TARGET_LONGITUDE = 139759252;

// 農学部広場
//const long TARGET_LATITUDE = 35715922;
...
//	RunUpNearGoal(3.0);
	...
	GoStraight(Deg2Rad(10), Meter2Msec(10), 100);
	delay(1000);
	GoStraight(Deg2Rad(100), Meter2Msec(10), 100);
	delay(1000);
	GoStraight(Deg2Rad(190), Meter2Msec(10), 100);
	delay(1000);
	GoStraight(Deg2Rad(280), Meter2Msec(10), 100);
	delay(1000);
	GoStraight(Deg2Rad(0), Meter2Msec(10), 100);
	delay(1000);
...
//		delay( Meter2Msec(3.0) );
		...
		goStraightMeter = max(Gps_Distance, 3);
		...
		SetMoter(255,0,255,0);
		GoStraight(Gps_Radian, Meter2Msec(goStraightMeter), 100);
...
int SetRoverAngle(double targetRad, int maxCount) {
	...
	if (DEBUG) Serial.println("NowRad deg : ");
...
void GoStraight(double targetRad, unsigned long msec, int loopTime) {
	...
	while(count < maxCount){
		...
		LogMagRaw();
		if (DEBUG) Serial.print("magnetRad deg : ");
		if (DEBUG) Serial.print( Rad2Deg(magnetRad)) ;
		if (DEBUG) Serial.print(",\t errorDirection : ");
		if (DEBUG) Serial.print(errorDirection);
...
//		Serial.println(Rad2Deg( errorDirection ),6);
		...
		if (DEBUG) Serial.print(",\t inputPid : ");
		if (DEBUG) Serial.println(inputPid);
...
int InitializeMagnet() {
	...
	int magnetCalError = 10;			// 地磁気キャリブレーションの許容エラー
