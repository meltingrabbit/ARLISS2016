...
double MagnetCalibration(double magnetRad);
// GPS系
int InitializeGps();
...
void loop() {
	...
	Serial.println( Rad2Deg( NormalizeRadSym(magnetRad) ) );
...
double MagnetCalibration(double magnetRad){
	...
	magnetRad = NormalizeRad(magnetRad);		//ifがややこしくなるので0-2piにおさめる
	magnetDeg = Rad2Deg(magnetRad);

	if(magnetDeg >= 345.2 && magnetDeg <= 360){
		ans = 70 * NormalizeRad(magnetRad + Deg2Rad(17.5)) / 183.25;			//分母分子、pi/180打ち消しあい
	}
	else if(magnetDeg >= 0 && magnetDeg <= 165.75){
		ans = 70 * (magnetRad + Deg2Rad(17.5)) / 183.25;
	}
	else if(magnetDeg > 165.75 && magnetDeg < 231){
		ans = 130 * ((magnetRad + Deg2Rad(17.5)) - Deg2Rad(183.25)) / 65.25 + Deg2Rad(70);
	}
	else{
		ans = 160 * ((magnetRad + Deg2Rad(17.5)) - Deg2Rad(248.5)) / 111.5 + Deg2Rad(200);
	}
	return ans;
}
