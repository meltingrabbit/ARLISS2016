...
//	while(1) {
//		countSwitch = 0;
//		Serial.println("Waiting for starting...");
//		Serial.println(digitalRead(PIN_SWITCH));
//		for (int i = 0; i < 10; i++) {
//			countSwitch += digitalRead(PIN_SWITCH);
//			delay(50);
...
void OpenCasing() {
	...
	delay(2000);
	Serial.println("FET2 ON");
	digitalWrite(PIN_FET_2, HIGH);
	delay(4000);
	digitalWrite(PIN_FET_2, LOW);
	Serial.println("FET2 OFF");
	...
	GoStraight(magnetRad, Meter2Msec(5));
	delay(1000);
	...
	while(1) {
		...
		if (Gps_Distance > 10) {
			...
			if (deltaGpsDistance < 5) {
				...
				ToneAlert();
				if (DEBUG) Serial.println("STUCK !!! HELP ME !!!");
				BurstSpin(1, 3000);
				delay(1000);
				BurstSpin(-1, 3000);
				delay(1000);
				double absTemp = 0;
				while (absTemp < Deg2Rad(45.0)) {
					BurstSpin(1, 100);
...
int MagnetCalibration() {
	...
	const int moterPower = 170;
	const unsigned long measuringTime = 5000;
	unsigned long time;
