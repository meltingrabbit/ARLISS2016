...
//	MY_FILE.print("! Magnet X Y\n");
//	MY_FILE.print("\n");
//	int magnetTry = 1;
//	while (1) {
//		int mc = MagnetCalibration();
//		MY_FILE.print("! MAGNET_TRY_NUM : ");
//		MY_FILE.println(magnetTry);
//		MY_FILE.print("! MAGNET_CAL_COUNT : ");
//		MY_FILE.println(mc);
//		MY_FILE.print("! MAGNET_X_OFFSET : ");
//		MY_FILE.println(MAGNET_X_OFFSET);
//		MY_FILE.print("! MAGNET_Y_OFFSET : ");
//		MY_FILE.println(MAGNET_Y_OFFSET);
//		MY_FILE.print("! MAGNET_XY_RATIO : ");
//		MY_FILE.println(MAGNET_XY_RATIO);
//
//		Serial.print("! MAGNET_TRY_NUM : ");
//		Serial.println(magnetTry);
//		Serial.print("! MAGNET_CAL_COUNT : ");
//		Serial.println(mc);
...
void loop() {
	...
	MY_FILE.print("\t");
	MY_FILE.print(Magnet_X12 + MAGNET_X_OFFSET);
	MY_FILE.print("\t");
	MY_FILE.print( (Magnet_Y12 + MAGNET_Y_OFFSET)*MAGNET_XY_RATIO );
	MY_FILE.print("\n");


	Serial.print(millis());
	Serial.print("\t");
	Serial.print(Magnet_X12 + MAGNET_X_OFFSET);
//	Serial.print(Magnet_X12);
	Serial.print("\t");
	Serial.print( (Magnet_Y12 + MAGNET_Y_OFFSET)*MAGNET_XY_RATIO );
...
void RunUpNearGoal(double residualError) {
	...
	while(1) {
		...
		if (DEBUG) Serial.println( Rad2Deg( NormalizeRadSym( GetMagnetRad() ) ) );
		if (DEBUG) Serial.print("#NowRad to deg : ");
		if (DEBUG) Serial.println( Rad2Deg( NormalizeRadSym( GetMagnetRad() ) ) );
		delay(1000);
...
int SetRoverAngle(double targetRad, int maxCount) {
	...
	double epsilonRad = Deg2Rad(30.0);	// 許容誤差 注意スべきは，許容誤差が最小角度変更より小さいと発振
	double dRad;
	int count = 0;
...
int InitializeMagnet() {
	...
	int magnetCalError = 3;			// 地磁気キャリブレーションの許容エラー
...
int MagnetCalibration() {
	...
	MAGNET_XY_RATIO = (xMax - xMin) / (yMax - yMin);
...
int CacheOff() {
	...
	if (DEBUG) Serial.println("Cache to SD");
	...
	Cache_End_Address = Cache_Start_Address;
	Cache_Size = 0;
	Is_Cache_Full = 0;
	return 1;
}
...
int LogCache(const char *string) {
	...
	if ( (Cache_Size*3) > (MAX_EEPROM_ADDRESS*4) ) {
		Is_Cache_Full = 1;
		return 0;
	} else {
		return 1;
...
int LogMagCal(int maxRange, int *xMaxs, int *yMaxs, int *xMins, int *yMins, int arrLen) {
	// 配列の長さを変更する場合，下のfor内も変更すること忘れずに
	char stringXOffset[20];
	char stringYOffset[20];
	char stringXYRatio[20];
	for (int i=0; i<20; i++) {
		stringXOffset[i] = '\0';
		stringYOffset[i] = '\0';
		stringXYRatio[i] = '\0';
	}
	sprintf(stringXOffset, "%.3f", MAGNET_X_OFFSET);
	sprintf(stringYOffset, "%.3f", MAGNET_Y_OFFSET);
	sprintf(stringXYRatio, "%.3f", MAGNET_XY_RATIO);
	...
	if (Log_To_Sd) {
		...
		MY_FILE.print(stringXOffset);
		MY_FILE.print(",");
		MY_FILE.print(stringYOffset);
		MY_FILE.print(",");
		MY_FILE.print(stringXYRatio);
		MY_FILE.print(",");
		...
		str = String(millis());
		str += String(",MagCal,");
		str += String(stringXOffset);
		str += String(",");
		str += String(stringYOffset);
		str += String(",");
		str += String(stringXYRatio);
		str += String(",");
		str += String(maxRange);
		str += String(",xMaxs");
		for (int i=0; i<arrLen; i++) {
