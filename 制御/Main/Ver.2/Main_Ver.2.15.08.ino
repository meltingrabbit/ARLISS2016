...
//着地判定
long LIGHT_JUDGE_DELAY_SEC = 3600;		//放出判定した場合、放出判定してから着地と判断するまでの時間
long LIGHT_JUDGE_TIMEOUT_SEC = 3600;
long GPS_LIGHT_JUDGE_TIMEOUT_SEC = 10800;		//単位は(秒) この二つのTIMEOUT_SECの和が、着地判定自体のタイムアウト時間
...
const long TARGET_LATITUDE = 35717713;
const long TARGET_LONGITUDE = 139759901;
...
void loop() {
	...
	/*	Serial.println(F("Waiting for CasingOK Cmd..."));
		time = millis();
		int cmd = 0;
		while (millis() < time + Sec2Msec(60 * 10) ) {		// コマンド打ち込み待ち
			cmd = ReceiveCmd();
			if (cmd == 2) {
	...
	CacheOn();
	Serial.println(F("RunUpNearGoal(10.0)"));
	RunUpNearGoal(10.0);
...
int JudgeLandingLightMode() {
	...
	while(1) {
		if (GetLightNTimes(20) == 1) {
			for (int i = 0; i < 30; i++) {
				Serial.println(F("waiting for 30s in JudgeLandingLightMode"));
				GetGpsData();
				LogGps();
				XbeeGps();
				GetLight();				// ログ用
...
void JudgeLandingGpsLightMode() {
	...
	while(1) {			//!!!gps判定ループ!!!
		...
		if (GetLightNTimes(20) == 1) {
			for (int i = 0; i < 30; i++) {
				Serial.println(F("waiting for 30s in JudgeLandingGpsLightMode"));
				GetGpsData();
				LogGps();
				XbeeGps();
				GetLight();				// ログ用
