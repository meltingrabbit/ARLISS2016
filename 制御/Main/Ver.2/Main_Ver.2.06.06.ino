...
void setup() {
	...
	// #####################################
	// int missionId = 0;
	// while ( !(SD.exists( (String(missionId) + String("/")).c_str() )) ) {
	// 	Serial.print(missionId);
	// 	Serial.println(F(" exist..."));
	// 	missionId++;
	// 	delay(2000);
	// }
	// SD.mkdir( (String(missionId) + String("/")).c_str() );
	// while(1) {}
	...
	Serial.println(missionDir);
	Serial.println(F("OK"));
	char fileName[] = "MD000000/LOG.CSV";
	for (int i = 0; i<8; i++) {
		fileName[i] = missionDir[i];
	}
...
void JudgeLandingGpsLightMode() {
	unsigned long time = millis();
	const int rangeGpsJudge = 2*3;		// 偶数にする 2*片側範囲
	const int averageNum = 30;			// 基準平均のサンプリング回数
	const int averageNumMin = 20;		// 基準平均のサンプリング最低回数
	const int sampleNum = 60;			// 判定に使うためのサンプリング回数
	const int sampleNumMin =40;			// 判定に使うためのサンプリング最低回数
	...
	if (DEBUG) Serial.print(F("GpsJudgeStart!"));


	while(GetLightNTimes(20) == 0) {			//1!!gps判定ループ!!!
		...
		while(count < averageNum) {				//!!!平均取得!!!
			if (DEBUG) Serial.println(F("Start GetGpsAverageLoop"));
		...
		if(countGpsData >= averageNumMin) {		//!!!判定準備!!!   (平均の時に一定数以上取れてたら)
			if (DEBUG) Serial.println(F("GetGpsAverageSuccess, start sampling"));
			...
			if(countGpsData >= sampleNumMin){		//!!!判定!!!
				...
				int rateNearGpsData = countNearGpsData * 1.0 /sampleNum;
