...
void setup() {
	...
	// CacheOnなので大丈夫になった．
	LogSta("Switch ON!");
	if (DEBUG) Serial.println(F("Switch ON!"));
	...
	if (error == 0) {
		...
		if (DEBUG) Serial.println(F("initialize SD failed!"));
		ToneAlert();
	} else {
		LogSta("Initialize SD OK!");
		if (DEBUG) Serial.println(F("Initialize SD OK!"));
	}

	InitializeMoter();
	LogSta("Initialize Moter OK!");
	if (DEBUG) Serial.println(F("Initialize Moter OK!"));
...
//	}
	...
	if (DEBUG) Serial.println(F("Initialize Magnet OK!"));
	...
	if (error == 0) {
		...
		if (DEBUG) Serial.println(F("initialize GPS failed!"));
		ToneAlert();
	} else {
		LogSta("Initialize GPS OK!");
		if (DEBUG) Serial.println(F("Initialize GPS OK!"));
	}

	// ###########################
	// 初期化完了の音を鳴らしたい
	LogSta("Initialization Finish!");
	if (DEBUG) Serial.println(F("Initialization Finish!"));
	for (int i = 0; i < 3; i++) {
...
void JudgeLanding(){
	...
	if (JudgeLandingLightMode() == 1) {
		...
		if ( GetLightNTimes(10) == 1) {			//安全のために30秒後にもう一回判定しておく
			...
			while (millis() < time + LIGHT_JUDGE_DELAY_SEC) {
				GpsBeacon();
				delay(2000);
			}
...
int JudgeLandingLightMode() {
	...
	int flag = 0;
	//ここで音
	while(GetLightNTimes(10) == 0) {
		if(millis() > time + Sec2Msec(LIGHT_JUDGE_TIMEOUT_SEC)) {
			return 0;
		}
		delay( Sec2Msec(5) );
	}
...
void JudgeLandingGpsLightMode() {
	...
	while(GetLightNTimes(10) == 0) {			//1!!gps判定ループ!!!
		...
		while(count < averageNum) {				//!!!平均取得!!!
			...
			if(GetLightNTimes(10) == 1) {					//平均取得中の光判定
				return;
			}
			XbeeGps();
			count++;
...
void GoStraight(double targetRad, unsigned long msec) {
	...
	int flag = 0;			//0のとき（かつout of controlRangeのとき)、片側を止める  controlRangeに一度でも入ったら1にする
	...
	while (millis() < startTime + msec) {
		...
		if (0 < errorDirection && errorDirection < controlRange) {
			...
			flag = 1;
		} else if (errorDirection >= controlRange && flag == 0) {
			SetMoter(255,0,0,0);
		} else if (errorDirection <= -controlRange && flag == 0) {
			SetMoter(0,0,255,0);
		} else {
...
//			if(DEBUG){
//				Serial.println(F("Reset I controller"));
//			}
			sumError = 0;
		}
...
int GetLightNTimes(int n) {			//N回すべておっけーなら判定
	while(n > 0) {
		if (GetLight() == 0) {
			return 0;
		}
...
int InitializeMagnet() {
	...
	int magnetCalError = 10;		// 地磁気キャリブレーションの許容エラー どんどん増やしていく
...
int InitializeGps() {
	...
	if (count > 0) {
		...
		if (DEBUG) Serial.println(F("GPS Sencer Dead!"));
		return 0;
	}
}
...
// ###########
//放出判定できずにGPS判定してる時の光判定の後delay挟むべき??