...
// カメラ
#define CameraSerialBegin(baudrate) Serial3.begin(baudrate)
#define CameraSerialRead() Serial3.read()
#define CameraSerialAvailable() Serial3.available()
#define CameraSerialPrint(string) Serial3.print(string)
#define CameraSerialSetTimeout(time) Serial3.setTimeout(time)
#define CameraSerialReadBytes(a, b) Serial3.readBytes(a, b)
...
// 大潟村ラグビーボール
// const long TARGET_LATITUDE = 40016036;
// const long TARGET_LONGITUDE = 139959215;
...
// 二日目
// 40.142684,139.987379
// const long TARGET_LATITUDE = 40142684;
// const long TARGET_LONGITUDE = 139987379;
// 40142716	139987291
// const long TARGET_LATITUDE = 40142716;
// const long TARGET_LONGITUDE = 139987291;
...
void loop() {
	...
	if (Mission_Status == 0) {
		...
		LogSta("Waiting for CasingOK Cmd...");
		time = millis();
		int cmd = 0;
		while (1) {		// コマンド打ち込み待ち
			cmd = ReceiveCmd();
		...
		time = millis();
		LogSta("waiting for LIGHT_JUDGE_START_DELAY_SEC");
		LogComStrLng("LIGHT_JUDGE_START_DELAY_SEC", LIGHT_JUDGE_START_DELAY_SEC);
...
void InitializeAll() {
	...
	LogSta("=========================");
	LogSta("=========================");
	Serial.println(F("InitializeAll"));
	LogSta("InitializeAll");
	...
	Serial.println(F("End InitializeAll"));
...
void CheckAndCalibrateSensor() {
	...
	if (SENSOR_CEHCK) {
		...
		Serial.println(F("CheckingSencor"));
		LogSta("CheckingSencor");
		...
		Serial.println(F("End CheckingSencor"));
		LogSta("End CheckingSencor");
...
void JudgeLanding() {
	...
	if (JudgeLandingLightMode() == 1) {
		...
		LogSta("waiting for LIGHT_JUDGE_DELAY_SEC");
		LogComStrLng("LIGHT_JUDGE_DELAY_SEC", LIGHT_JUDGE_DELAY_SEC);
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
			Serial.println(F("waiting for LIGHT_JUDGE_DELAY_SEC"));
			ClearGetGpsData();
			LogGps();
	...
	Serial.println(F("End JudgeLanding"));
	LogSta("End JudgeLanding");
}
	...
	while(1) {
		if (GetLightNTimes(20) == 1) {
			...
			if (GetLightNTimes(10) == 1) {			//安全のために30秒後にもう一回判定しておく
				Serial.println(F("JudgeLandingLightMode OK END!"));
				LogSta("JudgeLandingLightMode OK END!");
				return 1;
			}
		}
	...
	while(1) {			//!!!gps判定ループ!!!
		...
		if (GetLightNTimes(20) == 1) {
			...
			if (GetLightNTimes(10) == 1) {			//安全のために30秒後にもう一回判定しておく
				...
				Serial.println(F("JudgeLandingGpsLightMode Light OK END!"));
				LogSta("JudgeLandingGpsLightMode Light OK END!");
				return;
			}
		}
		...
		if (countGpsData >= averageNumMin) {		//!!!判定準備!!!   (平均の時に一定数以上取れてたら)
			...
			if(countGpsData >= sampleNumMin){		//!!!判定!!!
				...
				if(rateNearGpsData >= rateJudge){
					...
					Serial.println(F("GpsJudgeFailed..."));
					LogSta("GpsJudgeFailed...");
				}
			} else {
				Serial.println(F("GpsJudgeFailed...(get GPS failed)"));
				LogSta("GpsJudgeFailed...(get GPS failed)");
			}
		} else {
			Serial.println(F("GetGpsAverageFailed..."));
			LogSta("GetGpsAverageFailed...");
		}
...
void OpenCasing() {
	...
	Serial.println(F("End OpenCasing"));
...
void EscapeLandingPoint() {
	...
	Serial.println(F("End EscapeLandingPoint"));
	LogSta("End EscapeLandingPoint");

	if (DEBUG) Serial.println(F("OP : BurstGoAhead 5s"));
	BurstGoAhead(5000);
	MyDelay(20);

	// if (DEBUG) Serial.println(F("OP : GoStraight 1, 2m"));		for 能代コメントアウト
	magnetRad = NormalizeRadSym(GetMagnetRadNTimes(5));
	// GoStraight(magnetRad, Meter2Msec(2));			for 能代コメントアウト
	// MyDelay(1000);
	...
	if (DEBUG) Serial.println(F("OP : GO P 2, 2m"));
	...
	while(1) {
		...
		if (Gps_Distance > 6) {
			...
			deltaGpsDistance = abs(deltaGpsDistance);
				...
				LogComStrDbl("deltaGpsDistance", deltaGpsDistance);
		...
		LogComStrDbl("goStraightMeter", goStraightMeter);
...
void AvoidStuck() {
	...
	Serial.println(F("AvoidStuck"));
	LogSta("AvoidStuck");

	magnetRad = GetMagnetRadNTimes(5);

	if (flag < 7) {					//7:3で分岐 要調整
		Serial.println(F("AvoidStuck Type1"));
		LogSta("AvoidStuck Type1");
		...
		Serial.println(F("AvoidStuck Type2"));
		LogSta("AvoidStuck Type2");
	...
	Serial.println(F("End AvoidStuck"));
	LogSta("End AvoidStuck");
...
int SetRoverAngle(double targetRad, int maxCount) {
	...
	Serial.println(F("SetRoverAngle"));
	...
	Serial.println(F("End SetRoverAngle"));
	LogComStrLng("End SetRoverAngle", count);
...
void GoStraight(double targetRad, unsigned long msec) {
	...
	SetMoter(0,0,0,0);
	Serial.println(F("End GoStraight"));
	LogSta("End GoStraight");
	return;
}

// 正(1.0)で右，負(-1.0)で左
void SpinRover(double LorR, unsigned long msec) {
	Serial.println(F("SpinRover"));
	LogComStrDblLng("SpinRover", LorR, msec);
	if (LorR > 0) {
		SetMoter(255,0,0,255);
...
//		SetMoter(0,300,300,0);
	...
	MyDelay(msec);
	SetMoter(0,0,0,0);
	Serial.println(F("End SpinRover"));
	LogSta("End SpinRover");
}
...
void BurstGoAhead(long msec) {
	Serial.println(F("BurstGoAhead"));
	LogComStrLng("BurstGoAhead", msec);
	SetMoter(1000,0,1000,0);
	MyDelay(msec);
	SetMoter(0,0,0,0);
	Serial.println(F("End BurstGoAhead"));
	LogSta("End BurstGoAhead");
}

void BurstGoBack(long msec) {
	Serial.println(F("BurstGoBack"));
	LogComStrLng("BurstGoBack", msec);
	SetMoter(0,1000,0,1000);
	MyDelay(msec);
	SetMoter(0,0,0,0);
	Serial.println(F("End BurstGoBack"));
	LogSta("End BurstGoBack");
}

// 正(1.0)で右，負(-1.0)で左
void BurstSpin(double LorR, unsigned long msec) {
	Serial.println(F("BurstSpin"));
	LogComStrDblLng("BurstSpin", LorR, msec);
	if (LorR > 0) {
	...
	MyDelay(msec);
	SetMoter(0,0,0,0);
	Serial.println(F("End BurstSpin"));
	LogSta("End BurstSpin");
}
...
void CalcDistanceDirection(){
	...
	LogComStrDbl("Gps_Distance", Gps_Distance);
	LogComStrDbl("Gps_Radian", Gps_Radian);
	LogComStrDbl("Gps_Radian to Deg", Rad2Deg(Gps_Radian));
	return;
}
...
int InitializeCamera() {
	...
	while (1) {
		if (millis() > time + 10000) {		// タイムアウト
			Serial.println(F("InitializeCamera TIMEOUT..."));
			LogSta("InitializeCamera TIMEOUT...");
			return 0;
		}
		//clearRxBuf();
		sendCmd(cmd,6);
...
void CapturePic() {
	...
	if (err == 0) {
		LogPho(0);
		if (DEBUG) Serial.println(F("Error at Capture"));
		return;
	} else {
		GetData();
		LogPho(1);
	...
	noTone(PIN_BUZZER);
	CacheOn();
	Serial.println(F("End CapturePic"));
	LogSta("End CapturePic");
	return;
}
...
int preCapture() {
	...
	while (1) {
		if (millis() > time + 10000) {		// タイムアウト
			Serial.println(F("preCapture TIMEOUT..."));
			LogSta("preCapture TIMEOUT...");
			return 0;
		}
		clearRxBuf();
		sendCmd(cmd, 6);
...
int Capture() {
	...
	while (1) {
		if (millis() > time + 10000) {		// タイムアウト
			Serial.println(F("Capture TIMEOUT 1..."));
			LogSta("Capture TIMEOUT 1...");
			tone(PIN_BUZZER, TONE_FREQUENCY_SD_CAMERA);
			return 0;
		}
		clearRxBuf();
	...
	while (1) {
		if (millis() > time + 10000) {		// タイムアウト
			Serial.println(F("Capture TIMEOUT 2..."));
			LogSta("Capture TIMEOUT 2...");
			tone(PIN_BUZZER, TONE_FREQUENCY_SD_CAMERA);
			return 0;
		}
		clearRxBuf();
	...
	while (1) {
		if (millis() > time + 10000) {		// タイムアウト
			Serial.println(F("Capture TIMEOUT 3..."));
			LogSta("Capture TIMEOUT 3...");
			tone(PIN_BUZZER, TONE_FREQUENCY_SD_CAMERA);
			return 0;
		}
		clearRxBuf();
		...
		if (resp[0] == 0xaa && resp[1] == (0x0e | CAMERA_ADDRESS) && resp[2] == 0x04 && resp[4] == 0x00 && resp[5] == 0x00) {
			...
			if (resp[0] == 0xaa && resp[1] == (0x0a | CAMERA_ADDRESS) && resp[2] == 0x01) {
				...
				LogComStrDbl("picTotalLen", picTotalLen);
				if (picTotalLen > 100000) {
					if (DEBUG) Serial.println(F("picTotalLen is too long"));
					Serial.println(F("picTotalLen is too long"));
					LogSta("picTotalLen is too long");
					tone(PIN_BUZZER, TONE_FREQUENCY_SD_CAMERA);
					return 0;
				}
				break;
