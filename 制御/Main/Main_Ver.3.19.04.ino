...
long LIGHT_JUDGE_DELAY_SEC = 1800;		//放出判定した場合、放出判定してから着地と判断するまでの時間
long LIGHT_JUDGE_TIMEOUT_SEC = 7200;
long GPS_LIGHT_JUDGE_TIMEOUT_SEC = 3600;		//単位は(秒) この二つのTIMEOUT_SECの和が、着地判定自体のタイムアウト時間
// long LIGHT_JUDGE_START_DELAY_SEC = 1;		// ロケット封入から，光判定までの空き時間
// long LIGHT_JUDGE_DELAY_SEC = 10;		//放出判定した場合、放出判定してから着地と判断するまでの時間
// long LIGHT_JUDGE_TIMEOUT_SEC = 1;
// long GPS_LIGHT_JUDGE_TIMEOUT_SEC = 180;		//単位は(秒) この二つのTIMEOUT_SECの和が、着地判定自体のタイムアウト時間
...
void JudgeLanding() {
	...
	if (JudgeLandingLightMode() == 1) {
		...
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
			...
			flag = JudgeLandingLightModeCheckGpsHeight();
			if (flag == 1) {
				break;
			}
...
void ApproachGoal(double residualError) {
	...
	while(1) {
		...
		// 進む
		goStraightMeter = Gps_Distance * 0.81;
