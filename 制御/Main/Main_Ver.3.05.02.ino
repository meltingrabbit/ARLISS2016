...
void MyPrintRemainingTimeln(unsigned long standardTimeMsec, long timeLimitSec);
// #########################################
...
void setup() {
	...
	MyPrintln(F("Initialization Finish!"));
	for (int i = 0; i < 3; i++) {
...
void loop() {
	...
	// 西側
	long passingLatitudes1[] = {40142716, 40142716};
	long passingLongitudes1[] = {139987291, 139987291};
	// 東側
	long passingLatitudes2[] = {40142716, 40142716};
	long passingLongitudes2[] = {139987291, 139987291};
	...
	if (Mission_Status == 0) {
		...
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_START_DELAY_SEC) ) {			// ロケット封入から判定開始までのディレイ
			MyPrintRemainingTimeln(time, LIGHT_JUDGE_START_DELAY_SEC);
...
void JudgeLanding() {
	...
	if (JudgeLandingLightMode() == 1) {
		...
		while (millis() < time + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
			MyPrintRemainingTimeln(time, LIGHT_JUDGE_DELAY_SEC);
...
int JudgeLandingLightMode() {
	...
	while(1) {
		...
		MyPrintRemainingTimeln(time, LIGHT_JUDGE_TIMEOUT_SEC);
...
void JudgeLandingGpsLightMode() {		//ver.3.04.01から三軸完全一致
	...
	while(1) {			//!!!gps判定ループ!!!
		...
		if (GetLightNTimes(20) == 1) {
			...
			if (GetLightNTimes(10) == 1) {			//安全のために30秒後にもう一回判定しておく
				...
				while (millis() < lightTime + Sec2Msec(LIGHT_JUDGE_DELAY_SEC) ) {			//放出判定から着地判定までのディレイ
					MyPrintRemainingTimeln(lightTime,LIGHT_JUDGE_DELAY_SEC);
					MyPrintln(F("in LIGHT_JUDGE_DELAY_SEC"));
					ClearGetGpsData();
					LogGps();
					XbeeGps();
		...
		MyPrintRemainingTimeln(time, GPS_LIGHT_JUDGE_TIMEOUT_SEC);
		...
		if(countGpsData >= sampleNumMin){		//!!!判定!!!
			...
			if(rateMatchedGpsData >= rateJudge){
				...
				MyPrintln(F("JudgeLandingGpsLightMode GPS OK"));
...
void RunUpNearGoal(double residualError) {
	...
	LogComStrLng("Passing_Latitude", Passing_Latitude);
	LogComStrLng("Passing_Longitude", Passing_Longitude);
...
void ApproachGoal(double residualError) {
	...
	LogComStrDbl("ApproachGoal", residualError);
	LogComStrLng("TARGET_LATITUDE", TARGET_LATITUDE);
	LogComStrLng("TARGET_LONGITUDE", TARGET_LONGITUDE);
...
int RecoverRoverPosture(int maxTryCount) {
	...
	LogSta("RecoverRoverPosture failed...");
	return 0;
}
...
void CalcDistanceDirection(){
	...
	LogComStrDbl("Gps_Passing_Distance", Gps_Passing_Distance);
	LogComStrDbl("Gps_Passing_Radian", Gps_Passing_Radian);
	LogComStrDbl("Gps_Passing_Radian to Deg", Rad2Deg(Gps_Passing_Radian));
	return;
}
		...
		if (resp[0] == 0xaa && resp[1] == (0x0e | CAMERA_ADDRESS) && resp[2] == 0x04 && resp[4] == 0x00 && resp[5] == 0x00) {
			...
			if (resp[0] == 0xaa && resp[1] == (0x0a | CAMERA_ADDRESS) && resp[2] == 0x01) {
				...
				if (picTotalLen > 100000) {
					if (DEBUG) MyPrintln(F("picTotalLen is too long"));
...
void MyPrintRemainingTimeln(unsigned long standardTimeMsec, long timeLimitSec) {
	long remainingTimeSec = timeLimitSec + standardTimeMsec * 1.0 / 1000 - millis();
